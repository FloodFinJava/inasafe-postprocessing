    ...
    def run(self):
        ...
        for d in os.listdir(self.results_dir):
            ...
            if os.path.isdir(abspath):
                ...
                try:
                    loss_model = LossModel(self.conf, self.loss_curves, styling_path, impact_map_path)
                    loss_model.get_hazard_cats().get_cat_damage()
                    loss_model.estimate_damage().estimate_building_value().estimate_losses()
                    losses_by_area = loss_model.get_losses_by_area().rename(d)
...
def main():
    loss_model = LossEstimate(CONF_FILE)
    loss_model.run()
    loss_model.summary_to_csv()
    return None
